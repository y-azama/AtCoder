class AHC036:
  ...
  def move(self, pos_from, pos_to, pos_to_next = -1):
    L_A = self.param[3]
    L_B = self.param[4]
    ...
    while len(path) > 0:
      ...
      if pos_to_next > -1 and len(path) < L_B:
        next_path = self.search_path(pos_to, pos_to_next)
      dif_cnt = 0
      dif_cnt_max = 0
      dif_max_idx = -1
      last_idx = -1
      ...
      for i in indexes_a:
        ...
        for j in range(l,r):
          ...
          if tmp > common_cnt:
            ...
            cut_idx = j
          if tmp == common_cnt:
            tmp2 = len(set(next_path) & set(self.A[j:j+dif_cnt_max]))
            if tmp2 > common_cnt_next:
              common_cnt_next = tmp2
              cut_idx = j
      ...
      path = self.search_path(p, pos_to)
  ...
  def init_A(self, t):
    ...
    self.A = path.copy()
    for p in next_path[next_path_idx:]:
      if p not in self.A:
        self.A.append(p)
    
    tmp_q = []
    i = 0
    while len(self.A) < L_A:
      if len(tmp_q) == 0:
        j = i
        for i in range(j,N):
          if i not in self.A:
    ...
    if next_path_idx > -1:
      self.move(next_path[next_path_idx-1], next_path[-1], t[return_idx + 1])
...
for i, pos_to in enumerate(t[start_idx:]):
  if i + start_idx + 1 == len(t):
    solv.move(pos_from, pos_to)
  else:
    solv.move(pos_from, pos_to, t[start_idx + i + 1])
  pos_from = pos_to
